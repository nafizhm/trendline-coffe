        ...
        <section class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <h2 class="mt-2 text-3xl font-black text-slate-900">Edit Konten</h2>
                    <p class="mt-3 text-sm leading-7 text-slate-600">Perbarui data konten dari halaman ini. Konten bisa dikelola lebih dari satu item, tetapi aksi yang tersedia tetap hanya tambah dan edit.</p>
                </div>
                <a href="{{ route('contents.index') }}" class="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50">
                    Kembali
                </a>
            </div>
        </section>

        <form action="{{ route('contents.update', $contentItem) }}" method="POST" class="space-y-6" data-loading-form>
            @csrf
            @method('PUT')

            <section class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
                @include('contents._form', ['contentItem' => $contentItem])

                <div class="mt-6 flex justify-end gap-3">
                    <a href="{{ route('contents.index') }}" class="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                        Batal
                    </a>
